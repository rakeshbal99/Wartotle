...
void Search_Method(int m, int n, int o,int p_x,int p_y){
	...
	if(grid_bound(right)){
		state[right.x][right.y]=decode(right.x,right.y);
	}

	if(grid_bound(left)){
		state[left.x][left.y]=decode(left.x,left.y);
	}

	if(grid_bound(up)){
		state[up.x][up.y]=decode(up.x,up.y);
	}
	if(grid_bound(down)){
		state[down.x][down.y]=decode(down.x,down.y);
	}
	//up_right
	if(grid_bound(up_right)){
		state[up_right.x][up_right.y]=decode(up_right.x,up_right.y);
	}
	if(grid_bound(up_left)){
		state[up_left.x][up_left.y]=decode(up_left.x,up_left.y);
	}
	if(grid_bound(down_right)){
		state[down_right.x][down_right.y]=decode(down_right.x,down_right.y);
	}
	if(grid_bound(down_left)){
		state[down_left.x][down_left.y]=decode(down_left.x,down_left.y);
	}
