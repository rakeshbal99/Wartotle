...
void Search_Method(int m, int n, int o,int p_x,int p_y){
	...
	state[2][1]=(grid_bound(right))?decode(right.x,right.y):5;

	state[0][1]=(grid_bound(left))?decode(left.x,left.y):5;

	state[1][2]=(grid_bound(up))?decode(up.x,up.y):5;
	state[1][0]=(grid_bound(down))?decode(down.x,down.y):5;

	state[2][2]=(grid_bound(up_right))?decode(up_right.x,up_right.y):5;

	state[0][2]=(grid_bound(up_left))?decode(up_left.x,up_left.y):5;
	state[2][0]=(grid_bound(down_right))?decode(down_right.x,down_right.y):5;
	state[0][0]=(grid_bound(down_left))?decode(down_left.x,down_left.y):5;
		...
		)
	{
		...
		if (grid_bound(up) && state[1][2]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(right) && state[2][1]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(left) && state[0][1]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(down) && state[1][0]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(up_right) && state[2][2]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(up_left) && state[0][2]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(down_right) && state[2][0]==0 && visit[m][n][o]==0){
		...
		if (grid_bound(down_left) && state[0][0]==0 && visit[m][n][o]==0){
...
int main(int argc, char **argv)
{
	...
	outf<<"//initialise all visit rooms to zero\n";
